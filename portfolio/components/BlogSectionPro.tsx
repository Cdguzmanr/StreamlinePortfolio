...
const BlogSection: React.FC = () => {
  ...
  // Set up IntersectionObserver for active blog highlighting
  useEffect(() => {
    const observerOptions = {
      root: null,
      rootMargin: '-40% 0px -40% 0px', // Adjust for smoother transition near center
      threshold: 0.1,
    };
  
    const observer = new IntersectionObserver((entries) => {
      var mostVisible: { id: number; ratio: number } | null = null;
  
      entries.forEach((entry) => {
        const blogId = Number(entry.target.getAttribute("data-blog-id"));
        if (entry.isIntersecting) {
          if (!mostVisible || entry.intersectionRatio > mostVisible.ratio) {
            mostVisible = { id: blogId, ratio: entry.intersectionRatio };
          }
        }
      });
  
      if (mostVisible !== null) {
        setActiveBlog(mostVisible);
      }
    }, observerOptions);
  ...
  return (
    <div className="pb-12 bg-bone text-night">
                ...
                {visibleBlogs.map((blog) => (
                  <li key={blog.id}>
                    <button
                      ...
                      className={`w-full text-left text-[#6a6a6a] px-4 py-2 transition-all duration-100 flex items-center ${
                        activeBlog === blog.id
                          ? "bg-ucla-blue text-white"
                          : "text-[#616161] hover:bg-[#f3f3f3]"
                      }`}
                    >
      ...
      {/* <MobileBottomNav 
        visibleBlogs={visibleBlogs.map(({ id, short }) => ({ id, short }))}
